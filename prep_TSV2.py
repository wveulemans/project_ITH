				...
				for record in vcf_reader:
					...
					if int(pos) == variant_info_pos and chrom == variant_info_chrom and ref == variant_info_ref and alt == variant_info_alt:
						...
						if range_dict['begin']-2500 <= pos and int(pos) <= range_dict['end']+2500 and chrom == range_dict['chr']:
							...
							print '##################	not in gene'
							print filename, range_dict['gene_id'], range_dict['chr'], range_dict['begin'], pos, range_dict['end']
...
################################################################################################################################################################################
def prep_tsv2_all(name, range_pos_all, ALL):
	"""
		Input: 
		----------
	    	Function: 
	...
	#print glob.glob('/home/shared_data_core/COLON/subclonality/%s/pyclone*'% name)
	step1_files = glob.glob('/home/shared_data_core/COLON/subclonality/'+ALL+'/pyclone_*.filtered.vcf.tsv')
	#print step1_files
	...
	for var_id in patient_variant_dict:
		...
		for step1_file in step1_files:
			...
			if not filename in sample_list_ids:
				...
				if len(ref) > 1 or len(alt) > 1:
					vcf_file_sample = '/home/shared_data_core/COLON/subclonality/'+ALL+'/ith_'+('_'.join((step1_file.split('/')[6]).split('_')[2:7])).split('.')[0]+'.varscan2.pair.indel.filtered.vcf'
					snp_indel = 'indel'
				else:
					vcf_file_sample = '/home/shared_data_core/COLON/subclonality/'+ALL+'/ith_'+('_'.join((step1_file.split('/')[6]).split('_')[2:7])).split('.')[0]+'.varscan2.pair.snp.filtered.vcf'
					snp_indel = 'snp'
				...
				for record in vcf_reader:
					...
					if int(pos) == variant_info_pos and chrom == variant_info_chrom and ref == variant_info_ref and alt == variant_info_alt:
						...
						#print step1_file
						run = ('_'.join((step1_file.split('/')[6]).split('_')[2:8])).split('.')[0]
						#print run
						r = open('X.home.shared_data_core.COLON.ITH_nele_BAMs.ith_'+run+'.real.recal.summary.txt', 'rb')
						next(r)
